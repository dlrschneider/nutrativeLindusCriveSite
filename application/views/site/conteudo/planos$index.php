<div id="internas">
    <div class="main">
        <div class="container">
            <h1>Planos</h1>
            <div id="formContact" class="texto">
                O nutrative é uma opção incrivel tanto para nutricionistas quanto para seus clientes.
            </div>
            <div class="texto">
                <br><br>
                <div class="list">
                    <article>
                        <div class="icon">
                            <div class="icone">
                                <img src="img/site/layout/1.png">
                            </div>
                            <p class="tituloResumo">PLANO FREE</p>
                            <p class="resumoIcone"></p>
                            <ul>
                                <li>Cadastro de até 10 clientes</li>
                                <li>Até 20 publicações/men</li>
                            </ul>
                            <p></p>
                            <p class="resumoIcone">
                                <span>R$0,00 (sim, de graça!)</span>
                                <br>
                                <span>
                                    <select name="periodicidade1" class="periodicidade">
                                        <option value="">Sem cobranças</option>
                                    </select>
                                </span>
                                <br><br>
                                <a class="btPadrao2 laranja">assinar</a>
                            </p>
                        </div>
                    </article>
                    <article>
                        <div class="icon">
                            <div class="icone">
                                <img src="img/site/layout/2.png">
                            </div>
                            <p class="tituloResumo">PLANO II</p>
                            <p class="resumoIcone"></p>
                            <ul>
                                <li>Cadastro de até 50 clientes</li>
                                <li>Até 40 publicações/men</li>
                            </ul>
                            <p></p>
                            <p class="resumoIcone">
                                <span class="valor2">R$49,00/mês</span>
                                <br>
                                <span>
                                    <select name="periodicidade2" class="periodicidade" onchange="alterarPeriodicidade2(this.value)">
                                        <option value="Semestral">Cobrança semestral</option>
                                        <option value="Trimestral">Cobrança trimestral</option>
                                        <option value="Mensal">Cobrança mensal</option>
                                    </select>
                                </span>
                                <br><br>
                                <a class="btPadrao2 laranja">assinar</a>
                            </p>
                        </div>
                    </article>
                    <article>
                        <div class="icon">
                            <div class="icone">
                                <img src="img/site/layout/3.png">
                            </div>
                            <p class="tituloResumo">PLANO III</p>
                            <p class="resumoIcone"></p>
                            <ul>
                                <li>Cadastro de até 100 clientes</li>
                                <li>Até 120 publicações/men</li>
                            </ul>
                            <p></p>
                            <p class="resumoIcone">
                                <span class="valor3">R$109,00/mês</span>
                                <br>
                                <span>
                                    <select name="periodicidade3" class="periodicidade" onchange="alterarPeriodicidade3(this.value)">
                                        <option value="Semestral">Cobrança semestral</option>
                                        <option value="Trimestral">Cobrança trimestral</option>
                                        <option value="Mensal">Cobrança mensal</option>
                                    </select>
                                </span>
                                <br><br>
                                <a class="btPadrao2 laranja">assinar</a>
                            </p>
                        </div>
                    </article>
                    <article>
                        <div class="icon">
                            <div class="icone">
                                <img src="img/site/layout/4.png">
                            </div>
                            <p class="tituloResumo">PLANO IIII</p>
                            <p class="resumoIcone"></p>
                            <ul>
                                <li>Cadastro ilimitado de clientes</li>
                                <li>Até 200 publicações/men</li>
                            </ul>
                            <p></p>
                            <p class="resumoIcone">
                                <span class="valor4">R$189,00/mês</span>
                                <br>
                                <span>
                                    <select name="periodicidade4" class="periodicidade" onchange="alterarPeriodicidade4(this.value)">
                                        <option value="Semestral">Cobrança semestral</option>
                                        <option value="Trimestral">Cobrança trimestral</option>
                                        <option value="Mensal">Cobrança mensal</option>
                                    </select>
                                </span>
                                <br><br>
                                <a class="btPadrao2 laranja">assinar</a>
                            </p>
                        </div>
                    </article>
                </div>
            </div>
        </div>
    </div>  
</div>
